...
#       --num-demos 10
#
# Usage — fake SpaceMouse (CI / integration test, no hardware required):
#   cd RLinf
#   GENIESIM_ROOT=.. python examples/embodiment/collect_sim_data.py \
#       --config examples/embodiment/config/env/geniesim_junpu_place_workpiece.yaml \
#       --save-dir /tmp/sim_demos \
...
#       --fake-spacemouse
#
# SpaceMouse controls — Junpu task (real mode):
...
#   (No gripper button — Junpu collection does not use gripper-close on SpaceMouse.)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--fake-spacemouse",
        action="store_true",
        help="Use a deterministic fake SpaceMouse for testing (no hardware needed).",
    )
    p.add_argument(
        "--fake-move-steps",
        type=int,
        default=40,
        help="(fake mode) Steps of translational movement per episode.",
    )
    p.add_argument(
        "--fake-gripper-steps",
        type=int,
        default=15,
        help="(fake mode, legacy tests only) Ignored for Junpu collect — fake uses "
        "move+lift+left-button(save).",
    )
    p.add_argument(
        "--fake-lift-steps",
        type=int,
        default=40,
        help="(fake mode) Steps of lift movement per episode.",
    )
    p.add_argument(
        "--step-hz",
        type=float,
...
def main():
    ...
    if args.fake_spacemouse:
        expert = FakeSpaceMouseExpert(
            move_steps=args.fake_move_steps,
            lift_steps=args.fake_lift_steps,
            button_semantics="junpu",
        )
        print("[collect_sim_data] Using FakeSpaceMouseExpert (junpu) "
              f"(move={args.fake_move_steps}, lift={args.fake_lift_steps} "
              "then left button = save).")
    else:
        try:
            ...
            sys.exit(
                ...
                f"  Or use fake mode: add --fake-spacemouse to skip hardware.\n"
            )
        expert = SpaceMouseExpert()
        print("[collect_sim_data] Using real SpaceMouse device.")
        print()
        print("  Controls (Junpu):")
    ...
    env = CollectEpisode(
        ...
        only_success=True,   # only save episodes ended via left-button (junpu)
    )
