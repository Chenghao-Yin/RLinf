...
class SimProcessManager(BaseSimManager):
        ...
        if self.reuse_running:
            if self._ready_file.exists():
                if self._check_shm_accessible(shm_name):
                    print("[SimProcess] Reusing ready local sim_server process")
                    return
                print(
                    f"[SimProcess] Ready file exists but SHM '{shm_name}' not "
    ...
    def shutdown(self) -> None:
        ...
        if self.keep_alive:
            ...
            if not sim_active or self._idle_file.exists():
                return
            print(
                "[SimProcess] Stopping sim processes "
                "(sim_server.py stays alive for quick restart)...",
                flush=True,
            )
            ...
            while time.time() < deadline:
                if self._idle_file.exists():
                    print("[SimProcess] Local sim processes stopped.", flush=True)
                    return
                time.sleep(0.5)
            print(
                "[SimProcess] WARNING: local sim processes may not have stopped "
                "cleanly (60 s timeout).",
                flush=True,
            )
            return

        print("[SimProcess] Shutting down local sim_server.py...", flush=True)
        self._ready_file.unlink(missing_ok=True)
        if self._local_proc is not None and self._local_proc.poll() is None:
            self._local_proc.terminate()
            try:
                ...
                self._local_proc.kill()
            self._local_proc = None
            print("[SimProcess] Local sim_server.py stopped.", flush=True)
        self._kill_orphan_sim_servers()
    ...
    def _launch(self, pm_kwargs: Dict) -> None:
        ...
        sim_repo_root = self.geniesim_root / "main"
        sim_server_path = (
            sim_repo_root / "source" / "geniesim" / "rl" / "scripts" / "sim_server.py"
        )
        ...
        env.update({
            "SIM_REPO_ROOT":           str(sim_repo_root),
        ...
        print(
            f"[SimProcess] Starting local sim_server.py\n"
            f"[SimProcess]   sim_server    : {sim_server_path}\n"
            f"[SimProcess]   geniesim_root : {self.geniesim_root}\n"
            f"[SimProcess]   ros_ws_install: {self.ros_ws_install}\n"
            f"[SimProcess]   log           : {self._log_path}",
            flush=True,
        )
        ...
        for entry in Path("/proc").iterdir():
            ...
            if "sim_server.py" in cmdline:
                try:
                    ...
                    print(
                        f"[SimProcess] WARNING: cannot kill orphan sim_server "
                        f"pid={pid} (permission denied) — run `sudo kill -9 {pid}`"
                    )
        if killed:
            print(
                f"[SimProcess] Killed {len(killed)} orphan sim_server.py "
                f"process(es): {killed}",
                flush=True,
            )
            time.sleep(0.5)
