...
class JunpuPlaceWorkpieceEnv(GenieSimBaseEnv):
    """
    RLinf RL env for the Junpu pick-and-place task (right-arm only).
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._prev_dist_3d = None
        self._prev_orient_diff = None
        ...
        self._vis_enabled = self.cfg.init_params.get("vis_camera", False)
        ...
        if main is not None:
            ...
            if self._vis_enabled:
                try:
                    ...
                    cv2.imwrite("/tmp/geniesim_logs/env0_camera.jpg", frame_bgr)
                    cv2.imshow("env0_camera", frame_bgr)
                    cv2.waitKey(1)
                except Exception:
    ...
    def reset(self, seed=None, env_ids=None, **kwargs):
        ...
        if env_ids is None:
            ...
            if self._prev_dist_3d is not None:
                self._prev_dist_3d[ids] = -1.0
            if self._prev_orient_diff is not None:
                self._prev_orient_diff[ids] = -1.0
    ...
    def step(self, actions, auto_reset: bool = True):
        ...
        for i in range(n):
            if term[i] and not terminated[i]:
                ...
                print(f"[TERM] env_{i} (step {self._step_counter[i]}): {', '.join(reasons)}")
                rd = self._last_reward_detail
                print(f"  reward_detail: r_alive={rd['r_alive'][i]:.4f} r_appr={rd['r_approach'][i]:.4f} "
                      f"r_oappr={rd['r_orient_approach'][i]:.4f} r_spd={rd['r_speed'][i]:.4f} "
                      f"r_below={rd['r_below'][i]:.4f} d3d={rd['dist_3d'][i]:.4f} "
                      f"dxy={rd['dist_xy'][i]:.4f} dz={rd['diff_z'][i]:.4f} odiff={rd['orient_diff'][i]:.4f}")
    ...
    def _compute_reward(self, infos, states) -> torch.Tensor:
        ...
        r_approach = torch.zeros(n)
        r_orient_approach = torch.zeros(n)
        self._prev_dist_3d = dist_3d.clone()
        self._prev_orient_diff = orient_diff.clone()

        if isinstance(states, torch.Tensor):
            ee_lin_vel = states[:, 6:9]
        else:
            ee_lin_vel = torch.from_numpy(np.asarray(states)[:, 6:9])
        ee_speed = torch.linalg.norm(ee_lin_vel, dim=-1)
        excess_speed = torch.clamp(ee_speed - _EE_SPEED_THRESH, min=0.0)
        r_speed = torch.zeros(n)
        ...
        reward = r_alive + r_approach + r_orient_approach + r_speed + r_below + r_success
        ...
        self._last_reward_detail = {
            ...
            "r_orient_approach": r_orient_approach,
            "r_speed": r_speed,
            "r_below": r_below,
            "r_success": r_success,
            "dist_3d": dist_3d,
        ...
        try:
            s0 = states[0] if isinstance(states, torch.Tensor) else torch.from_numpy(np.asarray(states[0]))
            row = np.array([
                time.time(),
                float(self._step_counter[0]),
                float(wp_pos[0, 0]), float(wp_pos[0, 1]), float(wp_pos[0, 2]),
                float(s0[0]), float(s0[1]), float(s0[2]),
                float(s0[3]), float(s0[4]), float(s0[5]),
                float(s0[6]), float(s0[7]), float(s0[8]),
                float(s0[9]), float(s0[10]), float(s0[11]),
                float(dist_3d[0]), float(dist_xy[0]), float(diff_z[0]),
                float(orient_diff[0]),
                float(reward[0]),
                float(r_alive[0]), float(r_approach[0]),
                float(r_orient_approach[0]), float(r_below[0]),
                float(r_success[0]),
            ], dtype=np.float32)
