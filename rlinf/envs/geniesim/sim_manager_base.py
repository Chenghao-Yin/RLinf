...
class BaseSimManager(ABC):
    ...
    def _cleanup_stale_shm(self, shm_name: str) -> None:
        ...
        try:
            shm_path.unlink()
            print(f"[SimManager] Removed stale SHM '{shm_name}'")
        except PermissionError:
            owner_uid = shm_path.stat().st_uid
            print(
                f"[SimManager] WARNING: stale SHM '{shm_name}' owned by uid "
                f"{owner_uid} — run `sudo rm /dev/shm/{shm_name}` if startup fails"
            )
    ...
    def _cleanup_stale_ctrl_shms(self, shm_name: str) -> None:
        ...
        for entry in shm_dir.iterdir():
            ...
            try:
                entry.unlink()
                print(f"[SimManager] Removed stale ctrl SHM '{entry.name}'")
            except PermissionError:
                owner_uid = entry.stat().st_uid
                print(
                    f"[SimManager] WARNING: stale ctrl SHM '{entry.name}' "
    ...
    def _wait_ready(self) -> None:
        ...
        print(
            f"[SimManager] Waiting for simulation readiness "
        ...
        while time.time() < deadline:
            if self._ready_file.exists():
                ...
                print(f"[SimManager] Simulation ready ({elapsed:.0f}s elapsed)", flush=True)
                return
            ...
            try:
                ...
                if stage and stage != last_stage:
                    ...
                    elapsed = time.time() - t_start
                    print(f"[SimManager]   › {msg}  ({elapsed:.0f}s)", flush=True)
                    last_stage = stage
                    last_heartbeat = time.time()
            ...
            try:
                ...
                for line in log_tail.splitlines():
                    ...
                    if any(pat.lower() in stripped.lower() for pat in self._ISAAC_LOG_PATTERNS):
                        ...
                        print(f"[SimManager]     {stripped}", flush=True)
            except Exception:
                pass
